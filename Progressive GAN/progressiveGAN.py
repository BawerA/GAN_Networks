...
def build_generator_1():
    ...
    model.summary()    
    #gen_1_weights = model.get_weights()
    model.save_weights('gen1_weights')
    return model
...
def build_generator_2():
    ...
    # Input layer (100-d noise vector) fully-connected to 256 * 8 * 8 = 16,384 node dense layer
    model = models.Sequential()
    model.load_weights('gen1_weights')
    ...
    model.add(Conv2D(int(128), (5, 5), padding='same'))
...
for epoch in range(total_epochs):
    ...
    if epoch_counter == 0:
        ...
        combined_model3 = build_combined_3(generator3, discriminator3)                          
        combined_model3.compile(optimizer='adam', loss='binary_crossentropy')
    elif epoch_counter == 3:
        print("Next 5 epochs(15 and 20): \n")
        w = 4
        generator3 = build_generator_4()
        discriminator3 = build_discriminator_4()
        discriminator3.compile(optimizer='adam', loss='binary_crossentropy')
        combined_model3 = build_combined_4(generator4, discriminator4)                          
        combined_model3.compile(optimizer='adam', loss='binary_crossentropy')
    elif epoch_counter == 4:
        print("Next 5 epochs(20 and 25): \n")
        w = 5
        generator4 = build_generator_5()
        discriminator4 = build_discriminator_5()
        discriminator4.compile(optimizer='adam', loss='binary_crossentropy')
        combined_model4 = build_combined_5(generator5, discriminator5)                          
        combined_model4.compile(optimizer='adam', loss='binary_crossentropy')
    elif epoch_counter == 5:
        print("Next 5 epochs(25 and 30): \n")
        w = 6
        generator5 = build_generator_6()
        discriminator5 = build_discriminator_6()
        discriminator5.compile(optimizer='adam', loss='binary_crossentropy')
        combined_model5 = build_combined_6(generator6, discriminator6)                          
    ...
    for i in range(0, 1000, batch_size):
        ...
        if w == 1:
            ...
            fake_images.append(generator1.predict(noise)[the_variable])
            ...
            fake_images.append(generator2.predict(noise)[the_variable])
            ...
            fake_images.append(generator3.predict(noise)[the_variable])
            ...
            fake_images.append(generator6.predict(noise)[the_variable])# Sending noise vector to genereator
            ...
            x = np.vstack([fake, real])[shuffle_idx]   #Keep this commented, error here
            y = np.concatenate([np.ones(batch_size), np.zeros(batch_size)])[shuffle_idx]  #Keep this commented
            discriminator6.trainable = False#True 
...
plt.plot(loss[:, 1], label='Generato+r Loss')
plt.legend()
os.chdir(r"/media/root/New Volume/fake_images")
plt.savefig("Loss_Graph_Progressive_GAN.png")
