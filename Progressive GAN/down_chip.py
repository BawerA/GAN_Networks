...
def chip_images():
    ...
    per = 1
    X_data = []
    files2 = glob.glob ("/media/root/New Volume/carlos/8x8_chips/*.tif")#                     Change this to ur own directory
    files = glob.glob ("/media/root/New Volume/carlos/downsampling_testing/*.tif")#                     Change this to ur own directory
    for myFile in files:
        t = 0
        print('\nChipping image at this location: ', myFile)
        image = cv2.imread (myFile)
        ...
        while t < numberOfChips:
            #print(t + 1)
            os.chdir(r"/media/root/New Volume/carlos/8x8_chips") #       Change this to ur own directory
            mh.imsave('%d.tif' % per, chipped_img[t])
            os.chdir(r"/media/root/New Volume") #                     Change this to ur own directory
            t += 1
            per += 1
...
def get_16x16(batch_size):
    ...
    path_16x16 = glob.glob("/media/root/New Volume/carlos/16x16/*.tif")
    image_data_16x16 = []
    x = 0
...
def get_32x32(batch_size):
    os.chdir(r"/media/root/New Volume/carlos")
    path_32x32 = glob.glob("/media/root/New Volume/carlos/32x32_chips/*.tif")
    image_data_32x32 = []
    x = 0
    for x in range(x, batch_size):
...
def get_64x64(batch_size):
    os.chdir(r"/media/root/New Volume/carlos")
    path_64x64 = glob.glob("/media/root/New Volume/carlos/64x64_chips/*.tif")
    image_data_64x64 = []
    x = 0
    for x in range(x, batch_size):
...
def get_128x128(batch_size):
    os.chdir(r"/media/root/New Volume/carlos")
    path_128x128 = glob.glob("/media/root/New Volume/carlos/128x128_chips/*.tif")
    image_data_128x128 = []
    x = 0
    for x in range(x, batch_size):
...
def get_8x8(batch_size):
    os.chdir(r"/media/root/New Volume/carlos")
    path_8x8 = glob.glob("/media/root/New Volume/carlos/8x8_chips/*.tif")
    image_data_8x8 = []
    x = 0
    for x in range(x, batch_size):
