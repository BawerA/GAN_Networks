...
def chip_images():
    ...
    per = 1
    X_data = []
    files2 = glob.glob ("/Users/Bawer/Desktop/tensorboard/chipped_images/*.tif")#                     Change this to ur own directory
    files = glob.glob ("/Users/Bawer/Desktop/tensorboard/bawer_dataset/*.tif")#                     Change this to ur own directory
    for myFile in files:
        t = 0
        print('\nChipping image at this location: ', myFile)
        image = cv2.imread (myFile)
        ...
        while t < numberOfChips:
            #print(t + 1)
            os.chdir(r"/Users/Bawer/Desktop/tensorboard/chipped_images") #       Change this to ur own directory
            mh.imsave('%d.tif' % per, chipped_img[t])
            os.chdir(r"/Users/Bawer/Desktop/tensorboard") #                     Change this to ur own directory
            t += 1
            per += 1
...
def numpyify(batch_size):
    os.chdir(r"/Users/Bawer/Desktop/tensorboard")   
    path1 = glob.glob("/Users/Bawer/Desktop/tensorboard/chipped_images/*.tif")
    path2 = glob.glob("/Users/Bawer/Desktop/tensorboard/chipped_images2/*.tif")
    path_array = [path1, path2]
    image_data = []
    x = 0 
    #path_choice = random.choice(path_array)
    for x in range(x, batch_size):
        chipimage = mh.imread(random.choice(path1))
        image_data.append(chipimage)
    npchipped2 = np.array(image_data) 
    return npchipped2
